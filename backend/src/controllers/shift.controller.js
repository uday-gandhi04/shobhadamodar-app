...
const normalizeAtmEntries = (atmEntries = []) =>
  atmEntries
    .map((entry) => ({
...
const calculateCollections = (collections = {}) => {
  ...
  const atmEntries = normalizeAtmEntries(collections.atmEntries);

  const totalCardPaise = atmEntries.reduce(
...
const calculateFinalResult = async (shift, finalReadings, collections) => {
  ...
  for (const input of finalReadings) {
    ...
    if (!opening || !nozzle) {
      const error = new Error(
        `Nozzle ${input.nozzleId} is not part of this shift.`,
      );
      error.status = 400;
      throw error;
    }
    ...
    const dispensedLitres =
      Math.round((input.closingReading - opening.openingReading) * 100) / 100;
    const ratePaise =
  ...
  const differencePaise =
    financials.totalCollectedPaise - expectedTotalSalePaise;
...
export const getAvailableMpds = async (req, res, next) => {
  try {
    ...
    const lockedMpdIds = new Set(
      activeShifts.map((shift) => String(shift.mpdId)),
    );
...
export const startShift = async (req, res, next) => {
  try {
    ...
    const [existingEmployeeShift, existingMpdShift, mpd] = await Promise.all([
      ...
      Shift.findOne({ mpdId, status: "IN_PROGRESS" }).populate(
        "employeeId",
        "name employeeId",
      ),
...
export const previewEndShift = async (req, res, next) => {
  try {
    ...
    const result = await calculateFinalResult(
      shift,
      req.body.readings,
...
export const endShift = async (req, res, next) => {
  try {
    ...
    for (const updated of result.updatedNozzles) {
      const nozzle = result.mpd.nozzles.find(
        (item) => item.nozzleId === updated.nozzleId,
      );
      if (nozzle) {
    ...
    await Promise.all([
      shift.save(),
      Mpd.findByIdAndUpdate(result.mpd._id, { nozzles: result.mpd.nozzles }),
    ]);
