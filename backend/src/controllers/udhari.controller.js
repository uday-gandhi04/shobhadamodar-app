...
export const addUdhariTransaction = async (req, res, next) => {
  ...
  try {
    ...
    const matchingNozzle = mpd.nozzles.find(
    ...
    const ratePaise =
      fuelType === "PETROL" ? rates.petrolRatePaise : rates.dieselRatePaise;
    ...
    const hasLitres = litres !== undefined && litres !== null && litres !== "";

    const hasAmount =
      amountPaise !== undefined && amountPaise !== null && amountPaise !== "";

    if (!hasLitres && !hasAmount) {
      return res.status(400).json({
        success: false,
        message: "Enter either litres or rupee amount.",
        code: "AMOUNT_OR_LITRES_REQUIRED",
      });
    }

    if (hasLitres) {
      finalLitres = roundTwo(Number(litres));

      if (!Number.isFinite(finalLitres) || finalLitres <= 0) {
        return res.status(400).json({
          success: false,
      ...
      finalAmountPaise = Math.round(finalLitres * ratePaise);
    } else {
      finalAmountPaise = Math.round(Number(amountPaise));

      if (!Number.isFinite(finalAmountPaise) || finalAmountPaise <= 0) {
        return res.status(400).json({
          success: false,
          message: "Invalid rupee amount.",
          code: "INVALID_AMOUNT",
        });
      }

      finalLitres = roundTwo(finalAmountPaise / ratePaise);
    }
    ...
    await session.withTransaction(async () => {
       ...
       */
      const [createdTransaction] = await UdhariTransaction.create(
        [
          {
      ...
      customer = await Customer.findOneAndUpdate(
        ...
        {
          $inc: {
            outstandingBalance: finalAmountPaise,
          },
        },
        {
          new: true,
       ...
       */
      const shiftUpdate = await Shift.findById(shift._id).session(session);
      ...
      shiftUpdate.totalUdhariPaise += finalAmountPaise;
      ...
      shiftUpdate.reconciliationStatus = "PENDING";
      ...
      req.createdTransaction = createdTransaction;
      req.updatedShift = shiftUpdate;
    });

    return res.status(201).json({
      success: true,
      message: "Udhari added successfully.",
      data: {
...
export const deleteUdhariTransaction = async (req, res, next) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      const transaction = await UdhariTransaction.findOne({
      ...
      if (!Number.isInteger(amountPaise) || amountPaise <= 0) {
        const error = new Error("Udhari transaction has an invalid amount.");
        error.status = 409;
        throw error;
      }
      ...
      if (!customer) {
        const error = new Error("Customer balance cannot safely be reversed.");
        error.status = 409;
        throw error;
      }

      shift.udhariEntries = shift.udhariEntries.filter(
        (entry) => String(entry.transactionId) !== String(transaction._id),
      );
      ...
      const deleteResult = await UdhariTransaction.deleteOne({
        _id: transaction._id,
      }).session(session);

      if (deleteResult.deletedCount !== 1) {
        const error = new Error("Udhari transaction was already removed.");
        error.status = 409;
        throw error;
      }
